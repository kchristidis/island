...
The `blocknotifier` checks the ledger every `schema.SleepDuration` seconds for new blocks. If the received block marks the beginning of a new slot, it notifies the `slotnotifier`. The `blocknotifier` also invokes a dummy `Clock` method on the smart contract every `schema.ClockPeriod` seconds so as to ensure a continuous stream of blocks; this functionality is not necessary for the run to move forward however.

The `slotnotifier` is triggered by the `blocknotifier` when a new slot begins, and notifies all subscribed agents (bidders and the regulator) of that event. For experiments with a separate `PostKey` phase, a second `slotnotifier` is used to signal the beginning of that phase in a given slot.

The `statscollector` thread receives block statistics from the `blocknotifier` (what is the size of a block), and transaction statistics by the agents (what kind of transaction was invoked, what was its type, result, and end-to-end latency). At the end of the run, it queries the smart contract for slot statistics, and prints all three statistics to files in the `output` folder; see the "Parsing the results" section for more info.
